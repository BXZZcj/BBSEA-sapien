...
class SimplePickPlaceScene(TaskScene):
            ...
            if type(obj)==sapien.Actor and obj.get_name()!="table":
                ...
                for drawer_name in obj.get_drawer_name_list():
                    ...
                    node = Node(drawer_name + \
                                f" with open degree: {obj.get_open_distance_by_name(handle.get_name()):.2f} / {obj.get_open_limit_by_name(handle.get_name()):.2f}", 
                                drawer_pcd)
                    self.scenegraph.add_node_wo_state(node)
    ...
    def step(
        ...
        def _get_RGB(camera: CameraEntity, viewpoint: str):
            ...
            save_dir = os.path.join(self.get_subtask_dir(), viewpoint, "RGB")
        ...
        def _get_Depth(camera: CameraEntity, viewpoint: str):
            ...
            save_dir = os.path.join(self.get_subtask_dir(), viewpoint, "Depth")
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            save_file = str(self.step_index).zfill(5)+".png"
...
if __name__ == '__main__':
    ...
    # demo.primitives.DrawerOpen("StorageFurniture45290_handle_2")
    demo.primitives.Pick("banana")
    demo.primitives.PlaceOn("StorageFurniture45290_drawer_2")
