...
class SimplePickPlaceScene(TaskScene):
    def __init__(self):
        ...
        self.backward_camera = self._set_camera(
            ...
            camera_pose_origin=np.array([1.8, 0, 1]),
            camera_pose_target=np.array([0.2,0,0]),
            width=1600,
            height=1000,
            fovy=np.deg2rad(45),
        )
    ...
    def _create_tabletop(self) -> None:
        ...
        load_object_mesh(
            self, 
            sapien.Pose(p=[0.44, 0.58, 0.129699], q=euler.euler2quat(ai=0, aj=np.pi/2, ak=0)), 
            collision_file_path=os.path.join(manipulate_root_path, "assets/object/pepsi_bottle/visual_mesh.obj"),
            visual_file_path=os.path.join(manipulate_root_path, "assets/object/pepsi_bottle/visual_mesh.obj"),
            texture_file_path=os.path.join(manipulate_root_path, "assets/object/pepsi_bottle/texture.png"),
            name='pepsi_bottle',
        )
    ...
    def get_rgb_picture(self)->List[Image.Image]:
        cameras=[self.downward_camera, self.backward_camera]
        rgba_imgs=[]
        for camera in cameras:
            camera.take_picture()
            rgba = camera.get_float_texture("Color")
            rgba_array = (rgba * 255).clip(0, 255).astype("uint8")
            rgba_imgs.append(Image.fromarray(rgba_array))
...
if __name__ == '__main__':
    ...
    current_pose=demo.get_robot_by_name("panda_robot").body.get_links()[-4].get_pose()
    target_pose_p=current_pose.p+[-0.4,0,0.4]
    from utils.robot import panda_z_direction_quant
    target_pose_q=panda_z_direction_quant([0,0,-1])
    demo.primitives._move_to_pose(sapien.Pose(p=target_pose_p, q=target_pose_q))
    print(demo.get_robot_by_name("panda_robot").body.get_qpos())

    demo.backward_camera.take_picture()

    rgba = demo.backward_camera.get_float_texture("Color")
    rgba_img = (rgba * 255).clip(0, 255).astype("uint8")
    rgba_pil = Image.fromarray(rgba_img)
    ...
    # position = demo.downward_camera.get_float_texture('Position')
    # depth = -position[..., 2]
    # depth_img = (depth * 1000.0).astype(np.uint16)
    # depth_pil = Image.fromarray(depth_img)
